...
# -- Optional: pyttsx3 voice confirmation (SAPI5) -----------------------------
import threading as _threading
try:
    import pyttsx3 as _pyttsx3
    _tts_engine = _pyttsx3.init(driverName="sapi5")
    _tts_engine.setProperty("rate", 175)
    _tts_engine.setProperty("volume", 1.0)
    _TTS_OK = True
    print("  Voice confirmation available")
except Exception:
    _tts_engine = None
    _TTS_OK = False
    print("  pyttsx3 not available -- voice confirmation disabled")

def _speak(text: str):
    """Speak text in a background thread -- non-blocking."""
    if not _TTS_OK or _tts_engine is None:
        return
    def _run():
        try:
...
class DMRSessionDialog:
    ...
    def _create_ui(self):
        ...
        for i, (label, value) in enumerate(locations):
            p = left_locs if i < 4 else right_locs
    ...
    def validate_and_start(self):
        ...
        location_names = {
            ...
            'lower_back': 'Lower Back', 'left_knee': 'Left Knee',
...
# ============================================================================
# GESTURE CONTROLLER  --  Hands-free glove control (v3.2)
# ============================================================================
#
# Control scheme (Option B):
#   Modifier : Pinky (index 4) held above GESTURE_PINKY_KPA
#   Trigger  : Thumb (index 0) tapped while modifier is held
#
# State machine: IDLE -> ARMING -> ARMED -> COOLDOWN -> IDLE
# Command: Pinky hold + Thumb tap  ->  PAUSE / RESUME recording
#
# Reads sensor_data numpy array directly -- no new hardware or threads.
# ============================================================================

GESTURE_PINKY_KPA   = 30.0   # kPa: pinky must exceed this to arm modifier
GESTURE_THUMB_KPA   = 25.0   # kPa: thumb tap detection threshold
GESTURE_DWELL_MS    = 800    # ms:  pinky hold before modifier activates
GESTURE_COOLDOWN_MS = 2500   # ms:  lockout after command fires
GESTURE_TAP_MAX_MS  = 600    # ms:  thumb tap window


class GestureController:
    """
    Hands-free control via deliberate finger pressure gestures.
    Pinky hold (modifier) + Thumb tap (trigger) = Pause / Resume.
    Call tick() every 50 ms from update_loop().
    """
    ...
    def __init__(self,
                 ...
                 pinky_kpa   = GESTURE_PINKY_KPA,
                 thumb_kpa   = GESTURE_THUMB_KPA,
                 dwell_ms    = GESTURE_DWELL_MS,
                 cooldown_ms = GESTURE_COOLDOWN_MS):

        self._on_pause_resume = on_pause_resume
        self._pinky_kpa   = pinky_kpa
        self._thumb_kpa   = thumb_kpa
    ...
    def reset(self):
        ...
        self._elapsed_ms   = 0
        self._thumb_was_up = True

    def tick(self, sensor_array, tick_ms: int = 50):
        """
        Call every update tick.
        sensor_array: numpy array [Th, In, Md, Rg, Py] in kPa, index 0-4.
        """
        pinky = float(sensor_array[4])   # Py index
        thumb = float(sensor_array[0])   # Th index

        pinky_active = pinky >= self._pinky_kpa
        thumb_active = thumb >= self._thumb_kpa
        ...
        if self._state == self._IDLE:
            ...
            if not pinky_active:
                self._state = self._IDLE
            elif self._elapsed_ms >= self._dwell_ms:
                self._state        = self._ARMED
                self._elapsed_ms   = 0
                self._thumb_was_up = not thumb_active

        elif self._state == self._ARMED:
            if not pinky_active:
                self._state = self._IDLE
                return
            if self._thumb_was_up and thumb_active:
                if callable(self._on_pause_resume):
                    self._on_pause_resume()
                self._state      = self._COOLDOWN
                self._elapsed_ms = 0
                return
            self._thumb_was_up = not thumb_active
            if self._elapsed_ms >= self._cooldown_ms:
                self._state = self._IDLE
...
class TactileSenseClinical:
    ...
    def __init__(self, root):
        ...
        # Gesture controller -- hands-free pause/resume via pinky+thumb
        self.gesture_ctrl = GestureController(
            on_pause_resume = self._gesture_pause_resume)
    ...
    def _create_left_panel(self, parent):
        ...
        gest_box = ttk.LabelFrame(parent, text="✋ Gesture Control", padding=6)
        gest_box.pack(fill=tk.X, padx=5, pady=3)
        ttk.Label(gest_box, text="Pinky hold + Thumb tap",
                  font=('Arial', 7), foreground="gray").pack(anchor=tk.W)
        ttk.Label(gest_box, text="= Pause / Resume recording",
                  font=('Arial', 7), foreground="gray").pack(anchor=tk.W)
        self._lbl_gesture = ttk.Label(gest_box, text="READY",
                                       font=('Arial', 9, 'bold'),
                                       foreground="gray")
        self._lbl_gesture.pack(anchor=tk.W, pady=(4, 0))
    ...
    def update_loop(self):
        ...
        try:
            if self.sensor_mode != "disconnected":
                ...
                # Gesture controller -- runs every tick
                self.gesture_ctrl.tick(self.sensor_data, 50)
                if hasattr(self, "_lbl_gesture"):
                    if self.gesture_ctrl.is_armed:
                        self._lbl_gesture.config(
                            text="ARMED -- tap thumb", foreground="orange")
                    elif not self.is_recording and self.current_session_metadata:
                        self._lbl_gesture.config(
                            text="PAUSED", foreground="orange")
                    else:
                        self._lbl_gesture.config(
    ...
    def _gesture_pause_resume(self):
        """Fired by GestureController: pinky-hold + thumb-tap toggles recording."""
        if not self.current_session_metadata or self._session_saved:
            return   # no active session -- ignore
        self.toggle_record()
        # toggle_record() handles UI updates; just add voice confirmation
        if self.is_recording:
            _speak("Resuming")
        else:
            _speak("Paused")
